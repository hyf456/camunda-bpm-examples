...
public class FlowBackTest {
	...
	public void repositoryDeploy(){
		Deployment deploy = repositoryService.createDeployment()
				.addClasspathResource("processes/flowBack.bpmn")
				.name("测试流程回退-1")
				.deploy();
	...
	public void runtimeRelease(){
		...
		ProcessInstance pi = runtimeService.startProcessInstanceByKey("flowBack");
	...
	public void taskBack(){
		...
		//当前活动环节
		ActivityImpl currActivityImpl = definition.findActivity(task.getId());
		// FlowNode flowNode = (FlowNode) process.getFlowElement("_3");
		FlowNode flowNode = (FlowNode) bpmnModelInstance.getModelElementById("_3");
	...
	public void getNexNode(){
		...
		bpmnModelInstance.getDocumentElement();
		//获取当前节点输出连线

...
public class CatchingTest {
	...
	public void deploymentProcesses_zip() {
		Deployment deploy = repositoryService.createDeployment()
				...
				.addClasspathResource("processes/catching.bpmn")//指定zip完成部署
				.deploy();
	...
	public void startProcess() throws InterruptedException {
		...
		ProcessInstance parallel = runtimeService.startProcessInstanceByKey("catching");
		System.out.println("流程实例id:" + parallel.getId());
		System.out.println("流程定义id:" + parallel.getProcessDefinitionId());

		System.out.println("========================================================");
		List<Task> list = taskService.createTaskQuery().taskAssignee("catching1").list();
		for (Task task : list) {
		...
		for (Execution execution : executions) {
			System.out.println("执行流ID:" + execution);
			// 发送信号
			runtimeService.signalEventReceived("pay", execution.getId());
		}

		// 查询任务
		List<Task> list1 = taskService.createTaskQuery() // 创建任务查询
				.taskAssignee("catching2") // 指定某个人
				.list();
